<template>
  <div id="hello">
    ...
    <div style="margin-bottom:10px;float:right;">
      ...
      <el-popconfirm title="确定删除最后行吗？" @confirm="delLastRow">
		    <template #reference>
          <el-button type="primary" size="mini">删除末行</el-button>
		    </template>
      </el-popconfirm>
    </div>
  ...
  methods:{
    ...
    addColumn(idx) { // 新增列
      ...
      this.testDatas.map(p => { // 新增的对象无法被vue监听到
        // vue3无需 this.$set(p, obj.col, { content: '', show: true })
        p[obj.col] = {content: '', show: true}
      })
    },
    delColumn() { // 删除列
      this.showMenu = false
      let colKey = this.testCols[this.curColumn].col
